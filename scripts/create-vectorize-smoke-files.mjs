...
const LIVE_CONFIRM_FLAG = '--confirm-live-vectorize-smoke-embedding'
const root = process.cwd()
const buildDir = join(root, '.tmp', 'evidence-build')
const outputDir = join(root, '.tmp', 'vectorize-smoke')
...
} = require('../.tmp/evidence-build/rag/vectorize-smoke.js')

if (!process.argv.includes(LIVE_CONFIRM_FLAG)) {
  console.error(`Refusing to run live Workers AI smoke embedding. Re-run with ${LIVE_CONFIRM_FLAG} after confirming cost and account scope.`)
  process.exit(1)
}
...
if (!accountId || !apiToken) {
  console.error('CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN are required for the live smoke embedding.')
  process.exit(1)
}

const smokeRunId = `vectorize-smoke-${new Date().toISOString().replace(/[:.]/g, '-')}`
...
const summary = createVectorizeSmokeSummary({
  records,
  queryText: DEFAULT_VECTORIZE_SMOKE_QUERY,
  queryVector,
  smokeRunId
...
await writeFile(ndjsonPath, serializeVectorizeSmokeNdjson(records))
