...
const {
  MANUAL_LIVE_RAG_SMOKE_CONFIRM_FLAG,
  createManualLiveSmokeCases,
  createManualLiveSmokeRequestBody,
  evaluateManualLiveSmokeCase,
  readRagSsePayloads,
  summarizeRagSsePayloads
} = require('../.tmp/evidence-build/rag/manual-live-smoke.js')

if (!process.argv.includes(MANUAL_LIVE_RAG_SMOKE_CONFIRM_FLAG)) {
  console.error(`Refusing to run manual live RAG smoke. Re-run with ${MANUAL_LIVE_RAG_SMOKE_CONFIRM_FLAG} after confirming cost and access scope.`)
  process.exit(1)
}

const baseUrl = process.env.RAG_LIVE_SMOKE_URL
const accessKey = process.env.RAG_ACCESS_KEY

if (!baseUrl || !accessKey) {
  console.error('RAG_LIVE_SMOKE_URL and RAG_ACCESS_KEY are required. Do not print or commit the access key.')
  process.exit(1)
}
...
for (const testCase of createManualLiveSmokeCases()) {
  const response = await globalThis.fetch(url, {
    ...
    body: JSON.stringify(createManualLiveSmokeRequestBody(testCase))
  })
  ...
  const evaluation = evaluateManualLiveSmokeCase(testCase, summary)
...
const outputDir = join(root, '.tmp', 'manual-live-rag-smoke')
