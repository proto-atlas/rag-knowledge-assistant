...
test('トップページを開くとRAG確認画面の概要と検索フォームが見える', async ({ page }) => {
  await page.goto('/')

  await expect(page.getByRole('heading', { name: '架空文書を検索し、根拠付き回答を生成するRAG確認画面' })).toBeVisible()
...
test('確認用キーと質問を入力するとモック検索結果を表示する', async ({ page }) => {
...
test('検索結果からサーバー側の回答ストリームを生成できる', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'サーバー側の回答ストリーム' })).toBeVisible()
  await expect(page.getByText('申請期限に関する根拠候補では')).toBeVisible()
  await expect(page.getByText('[1]')).toBeVisible()
})

test('回答内の引用番号を押すと対応する根拠カードへフォーカス移動する', async ({ page }) => {
  await mockAskResponse(page, 'valid')
...
test('引用ID検証に失敗した回答は通常回答として表示しない', async ({ page }) => {
  ...
  await expect(page.getByText('不正な根拠ID: 99')).toBeVisible()
})
