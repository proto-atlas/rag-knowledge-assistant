...
} from '../../src/rag/vectorize-check'

describe('selectVectorizeCheckChunks', () => {
  it('確認用chunkを固定順で選びindexVersionをprovider用に置き換える', () => {
...
describe('createVectorizeCheckRecords', () => {
  it('Vectorize upsert用recordを作る', () => {
    const chunks = selectVectorizeCheckChunks(mockSearchCorpus).slice(0, 2)
  ...
  it('chunk数とvector数が違えば例外を投げる', () => {
    ...
    expect(() => createVectorizeCheckRecords({
      chunks,
      vectors: [
        [0.1, 0.2]
      ],
      checkRunId: 'check-2026-04-30'
    })).toThrow('check chunk and vector counts must match')
  })
})

describe('serializeVectorizeCheckNdjson', () => {
  it('Vectorize CLI用NDJSONを生成する', () => {
    expect(serializeVectorizeCheckNdjson([
      {
        id: 'chunk-1',
        values: [0.1, 0.2],
        metadata: {
    ...
    ])).toBe('{"id":"chunk-1","values":[0.1,0.2],"metadata":{"chunkId":"chunk-1","documentSlug":"doc-1","documentTitle":"Doc 1","category":"policy","indexVersion":"rag-bge-m3-v1","headingPath":["Heading"],"tags":["policy"],"checkRunId":"check"}}\n')
  })
})

describe('createVectorizeCheckSummary', () => {
  it('check概要を作る', () => {
    expect(createVectorizeCheckSummary({
      records: [
        {
          id: 'chunk-1',
          values: [0.1, 0.2],
