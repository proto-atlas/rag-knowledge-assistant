...
} from '../../src/rag/manual-live-check'

describe('createManualLiveCheckCases', () => {
  it('既知回答とno-answerの2ケースだけを返す', () => {
    expect(createManualLiveCheckCases()).toEqual([
      {
        id: 'known-answer',
...
describe('createManualLiveCheckRequestBody', () => {
  it('確認用キーをbodyに含めず質問とtopKだけを返す', () => {
    const body = createManualLiveCheckRequestBody({
      id: 'known-answer',
      question: 'リモート勤務の申請期限は？',
      topK: 5
    })
...
describe('evaluateManualLiveCheckCase', () => {
  it('known-answerでanswer_deltaとsourceがあれば成功にする', () => {
    const evaluation = evaluateManualLiveCheckCase({ id: 'known-answer' }, {
  ...
  it('no-answerでno_answerがありanswer_deltaがなければ成功にする', () => {
    const evaluation = evaluateManualLiveCheckCase({ id: 'no-answer' }, {
