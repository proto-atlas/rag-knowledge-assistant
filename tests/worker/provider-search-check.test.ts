...
import { runProviderSearchCheck } from '../../src/worker/provider-search-check'
...
describe('runProviderSearchCheck', () => {
  it('Workers AI、Vectorize、D1を結合してsource card用の検索結果を返す', async () => {
    ...
    await expect(runProviderSearchCheck(env)).resolves.toEqual({
      ok: true,
  ...
  it('provider search checkに必要なbindingが足りなければ例外を投げる', async () => {
    await expect(runProviderSearchCheck({})).rejects.toThrow('provider search check bindings are missing')
  })
})
