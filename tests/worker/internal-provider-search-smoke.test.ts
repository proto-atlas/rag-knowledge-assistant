...
describe('POST /api/internal/provider-search-smoke', () => {
  it('明示フラグがない場合は404を返す', async () => {
    const response = await app.request('/api/internal/provider-search-smoke', {
      method: 'POST',
      headers: {
  ...
  it('admin access keyが正しいとprovider search smoke結果を返す', async () => {
    const response = await app.request('/api/internal/provider-search-smoke', {
      method: 'POST',
      headers: {
        authorization: 'Bearer admin-key'
