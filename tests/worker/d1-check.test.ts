...
import { runD1SourceCheck } from '../../src/worker/d1-check'
...
describe('runD1SourceCheck', () => {
  it('D1から既定chunkをactive indexVersionで取得して内容の概要を返す', async () => {
    ...
    await expect(runD1SourceCheck(env)).resolves.toEqual({
      ok: true,
  ...
  it('D1 bindingが足りなければ例外を投げる', async () => {
    await expect(runD1SourceCheck({})).rejects.toThrow('D1 check bindings are missing')
  })
})
