...
import { runProviderVectorizeCheck } from '../../src/worker/provider-check'
import type { WorkerBindings } from '../../src/worker/types'

describe('runProviderVectorizeCheck', () => {
  it('Workers AIで質問をembeddingしてVectorizeをmetadata filter付きでqueryする', async () => {
    ...
    await expect(runProviderVectorizeCheck(env)).resolves.toEqual({
      ok: true,
  ...
  it('provider bindingが足りなければ例外を投げる', async () => {
    await expect(runProviderVectorizeCheck({})).rejects.toThrow('provider check bindings are missing')
  })
})
