...
export function App() {
  ...
  async function handleSearchSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!canSearch) {
      ...
      setSearchError({
        code: 'bad_request',
        message: '確認用キーと3文字以上の質問を入力してください。'
      })
  ...
  async function handleServerAnswerStart() {
    if (!canSearch) {
      ...
      setAnswerError('確認用キーと3文字以上の質問を入力してください。')
    ...
    try {
      for await (const event of streamAskKnowledgeBase({
        ...
        if (event.type === 'source_validation_failed') {
          ...
          setAnswerText('')
          setAnswerError(`${event.message} 不正な根拠ID: ${event.invalidSourceIds.join(', ')}`)
    ...
    <main className="app-shell">
      <section className="hero" aria-labelledby="page-title">
        <div className="hero__content">
          ...
          <h1 id="page-title">架空文書を検索し、根拠付き回答を生成するRAG確認画面</h1>
          <p className="hero__lead">
            この公開画面では、架空文書一覧、RAG処理フロー、評価証跡の入口を確認できます。動的な検索と回答生成は、課金と乱用防止のため確認用キーで保護します。
          </p>
        </div>
      </section>
      ...
      <section className="section" aria-labelledby="flow-title">
        <div className="section__header">
          ...
          <p>確認用キー付きの検索と回答生成では、Workers AI、Vectorize、D1、Anthropicを通る実API確認用の経路を確認できます。</p>
        </div>
        <div className="flow">
        ...
        <div className="section__header">
          <div>
            ...
            <p className="section__note">
              mock lexical retrievalの固定fixture証跡です。確認用の実プロバイダー構成で使うWorkers AI、Vectorize、Claude APIの品質を直接証明するものではありません。
            </p>
          </div>
      ...
      <section className="section" aria-labelledby="search-title">
        ...
        <p className="section__note">
          確認用キー付きの検索はWorkers AI embedding、Vectorize検索、D1 source lookupを呼びます。回答生成は根拠が十分な場合だけClaude APIへ進みます。確認用キーはAPI呼び出し制限用で、ブラウザの状態だけに保持し、保存しません。
        </p>
        <form className="search-form" onSubmit={handleSearchSubmit}>
          <label htmlFor="access-key">確認用キー</label>
          <input
            autoComplete="off"
            id="access-key"
  ...
  if (props.status === 'idle') {
    return (
      <div className="search-panel search-panel--muted">
        確認用キーと質問を入力すると、実API確認用の根拠候補を確認できます。
      </div>
    )
  }
  ...
  if (props.status === 'unauthorized') {
    return (
      <div className="search-panel search-panel--error" role="alert">
        {props.error?.message ?? '確認用キーを確認してください。'}
      </div>
    )
  }
  ...
  return (
    <section className="answer-panel" aria-labelledby="answer-title">
      <div className="answer-panel__header">
        <div>
          <h3 id="answer-title">サーバー側の回答ストリーム</h3>
          <p>根拠候補をもとにClaude APIのストリーミング回答を受信し、根拠ID検証後のUI状態まで確認します。</p>
        </div>
...
function AnswerBody(props: {
  ...
  if (props.status === 'idle') {
    return <div className="answer-box answer-box--muted">検索結果を使ってモック回答生成を試せます。</div>
  }
  ...
  if (props.status === 'source-validation-failed') {
    return (
      <div className="answer-box answer-box--warning" role="alert">
        ...
        <p>質問を具体化するか、根拠カードだけを確認してください。</p>
      </div>
    )
  }
  ...
  if (props.status === 'error') {
    return (
      <div className="answer-box answer-box--error" role="alert">
        {props.error ?? 'モック回答生成に失敗しました。'}
      </div>
    )
  }

  if (props.status === 'aborted') {
    return <div className="answer-box answer-box--muted">モック回答生成を中断しました。</div>
  }
