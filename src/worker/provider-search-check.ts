...
export type ProviderSearchCheckResponse = {
  ok: true
  model: string
  indexVersion: string
  queryText: string
...
export async function runProviderSearchCheck(env: WorkerBindings): Promise<ProviderSearchCheckResponse> {
  if (!env.AI || !env.RAG_VECTOR_INDEX || !env.RAG_DB || !env.RAG_ACTIVE_INDEX_VERSION) {
    throw new Error('provider search check bindings are missing')
  }
