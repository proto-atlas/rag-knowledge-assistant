...
export function formatExternalBenchmarkMarkdown(payload: ExternalBenchmarkPayload): string {
  ...
  return `# 外部Benchmark Subset Eval Evidence
...
Result: ${payload.result.failed === 0 ? 'pass' : 'recorded with failures'}
...
Raw JSON is stored next to this file as \`external-benchmark-eval-${payload.generatedAt.slice(0, 10)}.json\`.
`
}
