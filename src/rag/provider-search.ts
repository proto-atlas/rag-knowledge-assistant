...
// このportfolio demoのfixture index向け暫定値。corpusやmetric変更時は再校正する。
...
function parseVectorizeMetadata(value: unknown): VectorizeMatchMetadata {
  ...
  copyStringField(record, metadata, 'smokeRunId')

  return metadata
}

function copyStringField<T extends 'chunkId' | 'documentSlug' | 'indexVersion' | 'smokeRunId'>(
