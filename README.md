...
embedding model、chunking strategy、Vectorize metric、source corpusを変える場合は、既存の `indexVersion` の意味を上書きしません。全chunkを新しい条件でre-embeddingし、D1のchunk / index run metadataとVectorize metadataの `indexVersion` を同じ新versionへ揃えたうえで、`RAG_ACTIVE_INDEX_VERSION` を切り替えます。詳細な更新順序は [Index Version Policy](docs/PROVIDER-BINDINGS.md#index-version-policy) を参照してください。
