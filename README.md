...
30 秒で見る場合は、公開URLのトップページから概要、架空文書、RAGフロー、評価サマリーを確認できます。

もう少し詳しく見る場合は、[docs/verification.md](./docs/verification.md) から確認範囲、確認用キーで保護している範囲、主な検証記録を追えます。README上の説明と検証記録の対応は [docs/evidence/INDEX.md](./docs/evidence/INDEX.md) にまとめています。
...
- 実プロバイダー検索評価
- Mr. TyDi Japanese dev subsetを使ったローカル字句検索の基準値
- 確認用キーとIP-based rate limit
- Lighthouse、axe-core、bundle size、SSE latency
- 画面スクリーンショットと手動の外部サービス接続確認

各検証は、その時点の構成と対象データに対する結果です。一般的なRAG品質や任意の文書集合への性能を示すものではありません。
